...
<button id="test-worker" class="btn btn-secondary btn-sm" style="margin-top:0.5rem;">
  🧪 Test Worker connection
</button>
<div id="test-result" style="margin-top:0.5rem; font-size:0.85rem;"></div>

<script>
document.addEventListener('DOMContentLoaded', function() {
  document.getElementById('test-worker').addEventListener('click', async function() {
    ...
    try {
      const response = await fetch('https://turnstile-verify.follhim.workers.dev', {
        ...
        body: JSON.stringify({ token: 'test-fake-token' })
      });
      const data = await response.json();
      resultDiv.innerText = '✅ Worker reachable. Response: ' + JSON.stringify(data);
      resultDiv.style.color = 'lightgreen';
    } catch (err) {
      resultDiv.innerText = '❌ Worker NOT reachable: ' + err.message;
