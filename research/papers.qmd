...
<a class="btn btn-success" id="cv-download-trigger" href="https://bitli.pro/2KRCT_c18bc349" style="cursor:pointer;">
    <i class="bi bi-file-earmark-pdf"></i> Download CV
</a>
<span id="count" class="badge bg-primary ms-2">0</span>

<!-- Turnstile modal overlay -->
<div id="turnstile-modal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.55); z-index:9999; align-items:center; justify-content:center;">
  <div style="background:#fff; padding:2rem 2.5rem; border-radius:0.75rem; text-align:center; box-shadow:0 8px 32px rgba(0,0,0,0.25); min-width:320px;">
    <p style="margin:0 0 1rem; font-weight:600; font-size:1rem;">Verify to download CV</p>
...
<script type="module">
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Send token to Worker for server-side validation
    ...
    document.getElementById('cv-download-trigger').addEventListener('click', async function(e) {
      e.preventDefault();
      const href = this.getAttribute('href');

      // Open tab immediately (synchronous, before any awaits)
      // so mobile browsers don't block it as a popup
      const tab = window.open('', '_blank');

      // 1. Run Turnstile challenge
      const result = await runChallenge();
      closeModal();

      if (!result.ok) {
        console.error('Turnstile challenge failed:', result.code);
        tab.close();
        turnstile.reset(window._tsWidgetId);
        return;
      }

      // 2. Verify token server-side via Worker
      const verified = await verifyWithWorker(result.token);
      if (verified) {
        await incrementCount();
        tab.location.href = href;  // navigate the already-open tab
      } else {
        console.error('Server-side token verification failed');
        tab.close();
      }

      // 3. Reset for next click
      turnstile.reset(window._tsWidgetId);
    });
  });
</script>
