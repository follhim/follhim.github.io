...
options(HTTPUserAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36")
...
# Get publication data with error handling
publications_raw <- get_publications(scholar_id)

# Check if publications data is valid
if (is.data.frame(publications_raw) && "year" %in% colnames(publications_raw)) {
  ...
  # Fallback: create dummy data or extract from citation years
  publication_data <- citation_data %>%
    select(year) %>%
    mutate(publications = 1) %>%  # Assume 1 publication per year as fallback
    group_by(year) %>%
