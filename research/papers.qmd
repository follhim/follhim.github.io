...
tryCatch({
  ...
  message("Fetched fresh citation data and saved to: ", citation_file)
  
}, error = function(e) {
  warning(paste("Error fetching citation data:", e$message))
  
  # Only use cached data as fallback if API fails
  if (file.exists(citation_file)) {
    citation_data <<- read.csv(citation_file)
    message("Using cached citation data as fallback")
  } else {
    stop("No cached data available and API call failed")
  }
})
