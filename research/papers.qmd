...
# ── Fetch citation data with cache fallback ────────────────────────────────────
citation_data <- tryCatch({
  result <- get_citation_history_serpapi(scholar_id, api_key)
  if (!is.null(result)) {
    save_cache(result, "citation_data")
    message("Citation data fetched from SerpApi and cache updated.")
  }
  result
}, error = function(e) {
  message("Error fetching citation data: ", e$message)
  NULL
})
...
if (is.null(citation_data)) {
  ...
  if (!is.null(citation_data)) {
    message("SerpApi returned no citation data — using cached data.")
  } else {
    stop("No citation data available and no cache found. Cannot render page.")
  }
}

# ── Fetch publication data with cache fallback ─────────────────────────────────
publication_data <- tryCatch({
  result <- get_publications_serpapi(scholar_id, api_key)
  if (!is.null(result)) {
    save_cache(result, "publication_data")
    message("Publication data fetched from SerpApi and cache updated.")
  }
  result
}, error = function(e) {
  message("Error fetching publication data: ", e$message)
  NULL
})
...
if (is.null(publication_data)) {
  ...
  if (!is.null(publication_data)) {
    message("SerpApi returned no publication data — using cached data.")
  } else {
    stop("No publication data available and no cache found. Cannot render page.")
  }
}
